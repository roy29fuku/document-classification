...
# brownコーパスのニュースカテゴリの文、簡易タグを使う
corpus = list(nltk.corpus.brown.tagged_sents(categories='news', tagset='universal'))

# 未知語を<UNK>に置換に変換
corpus = _get_unk_words_replaced_corpus(corpus)
...
# 訓練、評価データに分ける
train_data, test_data = _split_data(corpus)

# 単語、品詞の種類の取得
words, tags = _get_words_and_tags(corpus)
len_words = len(words)
len_tags = len(tags)
